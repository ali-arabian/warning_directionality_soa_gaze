...
```{r}
FLWM_P <- read.delim(here::here("GitHub/data/data.txt"))
```
...
shapiro_test_result_O <- shapiro.test(FLWM_P$FLWM)
...
```{r}
FLWM_P$warning_type <- as.factor(FLWM_P$warning_type)  # It makes warning type a factor

FLWM_P$soa_s <- scale(FLWM_P$soa, center = TRUE, scale = TRUE) # standardizing soa
...
model_FLWM_P <- glmer(FLWM ~ warning_type * soa_s + (1 | sub), 
                         data = FLWM_P, 
                         family = binomial(link = "logit"))

summary(model_FLWM_P)
# adding a calculation of an anova table 
Anova(model_FLWM_P, type = "III")

```

Now calculate for multicollinearity 
```{r}
check_collinearity (model_FLWM_P)
```
...
What is important to us is the interaction effect between warning types and SOA. Now, we estimate how SOA influences the probability of glancing on the FLWM for different warning types. Specifically, we examine whether the relationship between SOA and the probability of glancing on the FLWM differs between warning types by computing estimated trends (slopes).

```{r}
emtrends_results <- emtrends(model_FLWM_P, pairwise ~ warning_type, var = "soa_s")
...
plot <- ggplot(model_estimates, aes(x = soa, y = prob, fill = warning_type)) +
  ...
  scale_y_continuous(breaks = seq(0, 0.5, 0.1), limits = c(0, 0.5), name = "Probability of fixation to the FLWM") +
...
# Save plot
ggsave(
  filename = "C:/Users/tsaar/OneDrive - University of Leeds/PhD/Study3/Plots/PFFLWM_model_estimates.tiff",
  plot = plot,
  width = 10,
  height = 5,
  dpi = 300,
