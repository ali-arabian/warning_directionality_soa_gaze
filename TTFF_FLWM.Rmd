...
TTFF_FLWM_S <- F_S %>%
  dplyr::filter(!is.na(TTFF_FLWM))

```


Removing TTFF_FLWM of 0, which means drivers were already looking at the hazard position before it appears. This is because the binomial analysis showed no differences between the probability of TTFF_FLWM of 0 between warning direction ans soa.


```{r}
TTFF_FLWM_S <- TTFF_FLWM_S[TTFF_FLWM_S$TTFF_FLWM != 0, ]
```
...
shapiro_test_result_O <- shapiro.test(TTFF_FLWM_S$TTFF_FLWM)
print(shapiro_test_result_O)

ggplot(TTFF_FLWM_S, aes(x = TTFF_FLWM)) +
  geom_histogram(binwidth = 0.1, color = "black", fill = "blue") +
  labs(title = "Distribution of TTFF_FLWM", x = "Time To First Fixation", y = "Frequency")
...
```{r}
TTFF_FLWM_S$warning_type <- as.factor(TTFF_FLWM_S$warning_type)  # It makes warning type a factor
TTFF_FLWM_S$soa_s <- scale(TTFF_FLWM_S$soa, center = TRUE, scale = TRUE) # standardizing soa
```
...
```{r}
model_TTFF_FLWM_S <- glmer(TTFF_FLWM ~ warning_type * soa_s + (1 | sub), 
                     data = TTFF_FLWM_S, family = Gamma(link = "log"))

summary(model_TTFF_FLWM_S)

# adding a calculation of an anova table 
Anova(model_TTFF_FLWM_S, type = "III")

```

Now calculate for multicollinearity 
```{r}
check_collinearity (model_TTFF_FLWM_S)
```
...
What is important to us is the interaction effect between warning types and SOA. Now, we estimate how SOA influences TTFF_FLWM for different warning types. Specifically, we examine whether the relationship between SOA and TTFF_FLWM differs between warning types by computing estimated trends (slopes).

```{r}
emtrends_results <- emtrends(model_TTFF_FLWM_S, pairwise ~ warning_type, var = "soa_s")
...
trend_summary <- trend_summary %>%
  mutate(
    TTFF_FLWM_change_s = (exp(soa_s.trend) - 1 ), # Convert log slopes to reaction time (s)
    TTFF_FLWM_perc_change = (exp(soa_s.trend) - 1 ) * 100  #percentage change of reaction time
  )
...
TTFF_FLWM_S$warning_type <- factor(TTFF_FLWM_S$warning_type, levels = c("towards_free_lane", "towards_hazard", "non_directional"))
# labels
...
mean_data <- TTFF_FLWM_S %>%
  group_by(warning_type, soa) %>%
  summarise(mean_TTFF_FLWM = mean(TTFF_FLWM, na.rm = TRUE))

# Create the plot
p <- ggplot(TTFF_FLWM_S, aes(x = factor(soa), y = TTFF_FLWM)) +
  ...
  geom_line(data = mean_data, aes(x = factor(soa), y = mean_TTFF_FLWM, group = warning_type, color = warning_type), 
            linewidth = 0.8, linetype = "dashed", color = "#3b3b3b") + 
  
  # Customize the plot
  labs(title = "Warning Direction", 
       x = "SOA",
       y = "TTFF on the FLWM (s)",
       color = "Statistics") +  
...
ggsave(filename = "C:/Users/tsaar/OneDrive - University of Leeds/PhD/Study3/Plots/TTFF_FLWM_S.tiff", plot = p, width = 10, height = 7, dpi = 300, device = "tiff")
```
...
Now we use that data and the model to predict TTFF_FLWMs for each level of warning type and SOA and save it to a new data frame

```{r}
new_rt_data <- data.frame(rt = predict(model_TTFF_FLWM_S, re.form = NA, newdata = new_data)) %>%
...
```{r}
ggplot() +
  geom_point(TTFF_FLWM_S %>%
               group_by(warning_type, soa) %>%
               summarise(mean_TTFF_FLWM = mean(TTFF_FLWM, na.rm = TRUE)), mapping = aes(x = soa, y = mean_TTFF_FLWM), col = "red") + 
  geom_point(TTFF_FLWM_S, mapping = aes(x = soa, y = TTFF_FLWM), position = position_jitter(width = 20), alpha = .1) +
...
```{r}
sims <- sim(model_TTFF_FLWM_S, n.sims = 1000) # 1 simulate plausible parameter values
...
```{r}
ggplot() +
  geom_point(data = TTFF_FLWM_S %>% 
               group_by(warning_type, soa) %>% 
               summarise(mean_TTFF_FLWM = mean(TTFF_FLWM, na.rm = TRUE), .groups = "drop"), 
             mapping = aes(x = soa, y = mean_TTFF_FLWM), col = "red") + 
  geom_point(data = TTFF_FLWM_S, 
             mapping = aes(x = soa, y = TTFF_FLWM), 
...
ggplot() +
  # Mean TTFF_FLWM per SOA (Red Points)
  geom_point(data = TTFF_FLWM_S %>%
               group_by(warning_type, soa) %>%
               summarise(mean_TTFF_FLWM = mean(TTFF_FLWM, na.rm = TRUE), .groups = "drop"),
             mapping = aes(x = soa, y = mean_TTFF_FLWM), col = "red") + 
  
  # Individual TTFF_FLWM points (Jittered)
  geom_point(data = TTFF_FLWM_S, 
             mapping = aes(x = soa, y = TTFF_FLWM, col = warning_type), 
             position = position_jitter(width = 20), alpha = .1) +
...
TTFF_FLWM_S_plot <- ggplot() +
  # Mean TTFF_FLWM per SOA (Red Points)
  geom_point(data = TTFF_FLWM_S %>%
               group_by(warning_type, soa) %>%
               summarise(mean_TTFF_FLWM = mean(TTFF_FLWM, na.rm = TRUE), .groups = "drop"),
             mapping = aes(x = soa, y = mean_TTFF_FLWM), col = "red", size = 0.7) + 
...
# Save plot
ggsave("C:/Users/tsaar/OneDrive - University of Leeds/PhD/Study3/Plots/TTFF_FLWM_S_GLMM.tiff", 
       plot = TTFF_FLWM_S_plot, width = 16, height = 8, units = 'cm', dpi = 300, device = "tiff")
